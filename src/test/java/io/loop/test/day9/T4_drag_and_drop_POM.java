...
public class T4_drag_and_drop_POM {
    ...
    public void drop_here_test(){
        ...
        assertEquals(loopPracticeDragDropPage.bigCircle.getText(), "Drop here.", "Actual does not match with expected");
    }

    @Test
    public void now_drop_here_test(){
        actions.moveToElement(loopPracticeDragDropPage.smallCircle).clickAndHold().moveByOffset(0, -200).pause(3000).perform();
        assertEquals(loopPracticeDragDropPage.bigCircle.getText(), "Now drop...","Actual does not match with expected" );
    }

    @Test
    public void try_again_test(){
        actions.moveToElement(loopPracticeDragDropPage.smallCircle).clickAndHold().moveByOffset(100,100).release().perform();
        assertEquals(loopPracticeDragDropPage.bigCircle.getText(), "Try again!","Actual does not match with expected" );
    }
